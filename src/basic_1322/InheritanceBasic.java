...
public class InheritanceBasic {
	 ...
	 * 			: 여러 인터페이스에서 동일한 디폴트메서드가 존재할때..
	 * 				인터페이스를 구현한 쪽에서 오버라이드 해야 한다
	 * 			: 디폴트 메서드와 조상클래스의 메서드간 충골
	 * 				조상클래스의 메서드가 상속되고 디폴트 메서드는 무시된다
	 * 
	 * 18. 내부클래스 (inner class)
	 * 		- 내부클래스란?
	 * 			: 클래스 내에 선언된 클래스이다
	 * 			: 서로쉽게 접근할 수 있고 외부에서 볼때 불필요한 클래스를 감춤으로
	 * 				복잡성을 줄일수 있다는 장점이 있다
	 * 		- 인스턴스클래스(instance class)
	 * 			: 외부클래스의 인스턴스 멤버들과 관련된 작업을 한다
	 * 		- static클래스(static class)
	 * 			: 외부클래스의 클래스멤버, 클래스메서드와 관련된 작업을 한다
	 * 		- 지역클래스(local class)
	 * 			: 외부클래스의 메서드나 초기화블럭내에 선언되며 블럭 내에서만 사용할 수 있다
